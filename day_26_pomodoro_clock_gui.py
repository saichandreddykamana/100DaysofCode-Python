from tkinter import *
import time
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
check = ""
timer = None
# ---------------------------- TIMER RESET ------------------------------- #


def reset_timer():
    window.after_cancel(timer)
    canvas.itemconfig(timer_text, text="00:00")
    label.config(text="Timer")
    global check
    check = ""
    global reps
    reps = 0

# ---------------------------- TIMER MECHANISM ------------------------------- #


def start_timer():
    global reps
    reps += 1
    work_sec = WORK_MIN * 60
    short_sec = SHORT_BREAK_MIN * 60
    long_sec = LONG_BREAK_MIN * 60
    if reps % 8 == 0:
        label.config(text="Long Break", fg=RED)
        count_down(long_sec)
    elif reps % 2 == 0:
        label.config(text="Short Break", fg=RED)
        count_down(short_sec)
    else:
        label.config(text="Timer")
        global check
        check += " ✔"
        checkmark_1.config(text= check)
        count_down(work_sec)

# ---------------------------- COUNTDOWN MECHANISM ------------------------------- #


def count_down(count):
    global timer
    count_min = math.floor(count / 60)
    count_sec = count % 60
    if count_sec < 10:
        count_sec = f'0{count_sec}'
    canvas.itemconfig(timer_text, text=f'{count_min}:{count_sec}')
    if count >= 0:
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()
# ---------------------------- UI SETUP ------------------------------- #


window = Tk()
window.title("Pomodoro")
window.minsize(width=500, height=500)
window.config(bg=YELLOW)

label = Label(text="Timer", font=(FONT_NAME, 25, 'bold'), bg=YELLOW, fg=GREEN)
label.grid(column=1, row=0, padx=20, pady=20)

canvas = Canvas(width=220, height=224, bg=YELLOW, highlightthickness=0)
tomato_png = PhotoImage(file="tomato.png")
canvas.create_image(110, 112, image=tomato_png)
timer_text = canvas.create_text(110, 130, text="00:00", fill="white", font=(FONT_NAME, 30, 'bold'))
canvas.grid(column=1, row=1, padx=20, pady=40)

checkmark_1 = Label(text=check, font=(FONT_NAME, 15, 'bold'), bg=YELLOW, fg=GREEN)
checkmark_1.grid(column=1, row=2, padx=20, pady=20)

start_btn = Button(text="Start", font=(FONT_NAME, 15, 'bold'), command=start_timer)
start_btn.grid(column=0, row=3, padx=20, pady=20)

reset_btn = Button(text="Reset", font=(FONT_NAME, 15, 'bold'), command=reset_timer)
reset_btn.grid(column=3, row=3, padx=20, pady=20)

window.mainloop()
